...
interface NoteDao {
    ...
    fun pagingSource(query: String): PagingSource<Int, NoteEntity>

    @Query(
        "SELECT * FROM notes WHERE categoryId = :categoryId " +
            "AND (title LIKE '%' || :query || '%' COLLATE NOCASE " +
            "OR content LIKE '%' || :query || '%' COLLATE NOCASE) " +
            "ORDER BY updatedAt DESC",
    )
    fun pagingSource(categoryId: Long, query: String): PagingSource<Int, NoteEntity>
